...
temp_calculation as 
        (           
            ...
            join {{ref('stg_spree_feeds')}} as feed on feed_id = feed.id and feed.deleted_at is null
            join {{ref('stg_spree_vendors')}}  as vendor on feed.vendor_id = vendor.id and vendor.deleted_at is null
            join {{ref('base_warehouses')}} as w on w.landing_region_id = ow.landing_region_id and w.deleted_at is null
            join {{ref('stg_spree_regions')}} as r on r.id = w.landing_region_id
            left join prep_countryas as c on vendor.country = c.code

            where ow.deleted_at is null and ow.active = true 
        ),
...
last_one as
        (
            ...
            warehouse, 
            departure_date, 
            DENSE_RANK() OVER (PARTITION BY warehouse,region, origin,Supplier, supplier_id ORDER BY departure_date) AS departure_rank 
            from aggregated
            where departure_date >= current_date()
            group by origin,region, Supplier, warehouse,departure_date,supplier_id
        )
