WITH OfferPrices AS (
   SELECT 
    COALESCE((CASE WHEN p.approval = TRUE THEN p.price END), p.price) AS price,-- LAST approved price OR DEFAULT price
    p.stock_id,
  ...
  FROM {{ref('stg_specification_values')}} as s
  LEFT JOIN {{ref('stg_specifications')}} AS spec ON s.specification_id = spec.id
  GROUP BY s.stock_id
     )
SELECT
...
END
  ...
  -- need TO UPDATE it TO be ON LAST (updated_at) 
  p.name AS product,
  p.color,
...
FROM {{ref('stg_stocks')}} AS st
LEFT JOIN {{ref('stg_offers')}} AS o ON st.stockable_id = o.id
LEFT JOIN {{ref('stg_product')}} AS p ON st.product_id = p.id
LEFT JOIN {{ref('stg_growers')}} AS g ON g.id = p.grower_id
LEFT JOIN {{ref('stg_offer_templates')}} AS ot ON ot.id = o.offer_template_id
LEFT JOIN {{ref('stg_quantity_units')}} AS qu ON qu.id = st.quantity_unit_id
LEFT JOIN {{ref('stg_feeds')}} AS f ON f.id = ot.feed_id
LEFT JOIN {{ref('stg_accounts')}} AS a ON g.account_id = a.id --
--LEFT JOIN floranow.Floranow_ERP.suppliers AS s ON s.floranow_supplier_id = a.floranow_account_id
LEFT JOIN {{ref('int_supplier')}} AS s ON s.floranow_supplier_id = a.floranow_account_id
