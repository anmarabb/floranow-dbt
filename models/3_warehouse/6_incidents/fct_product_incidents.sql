...
left join {{ref('base_suppliers')}} as stg_suppliers on stg_suppliers.supplier_id = li.supplier_id
...
left join {{ref('stg_feed_sources')}} as fs on li.feed_source_id = fs.feed_source_id
left join {{ref('base_stocks')}} as stock on p.stock_id = stock.stock_id 
left join {{ref('base_warehouses')}} as w on w.warehouse_id = stock.warehouse_id
