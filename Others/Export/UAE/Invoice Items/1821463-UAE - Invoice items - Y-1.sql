...
where stg_users.financial_administration = 'UAE' and ii.status = 'APPROVED' and ii.deleted_at is null and  date_diff(cast(i.printed_at as date), cast(current_date() as date ), YEAR) = 1