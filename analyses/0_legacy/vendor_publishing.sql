...
max(date(o.departure_date_time)) over(partition by g.name)  as last_departure_date,
...
case when DATE_DIFF(cast (max(date(o.departure_date_time)) over(partition by g.name) as date), CAST(CURRENT_DATE() AS date) ,day) > 9 then 'active' else 'inactive' end as supplier_status,
